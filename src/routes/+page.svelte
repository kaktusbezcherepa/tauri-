<script>
  ...
  function playAudio() {
    if (audio) audio.play().catch(error => console.error('Error playing audio:', error));
  }

  $: if (audio && typeof volumeValue === 'number') setVolume(volumeValue);
...
{#if showModal}
  <div class="modal-backdrop" transition:fade={{ duration: 300 }}>
    <div class="modal-content" role="dialog" aria-modal="true">
      ...
      <input type="range" id="volume" min="0" max="100" bind:value={volumeValue}/>
